...
variable "GCP_CLIENT_MACHINE_TYPE" {
	...
	validation {
		condition = can(regex("n1-standard-4", var.GCP_CLIENT_MACHINE_TYPE)) || can(regex("n1-standard-2", var.GCP_CLIENT_MACHINE_TYPE))
		error_message = "GCP_CLIENT_MACHINE_TYPE must be one of (n1-standard-4 | n1-standard-2) types."
	}
}
