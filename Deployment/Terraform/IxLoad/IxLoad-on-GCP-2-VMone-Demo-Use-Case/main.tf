...
resource "google_compute_network" "GCP_TEST2_VPC_NETWORK" {
	name = "${var.GCP_LOGIN_ID_TAG}-${var.GCP_PROJECT_TAG}-${var.GCP_TEST2_VPC_NETWORK_NAME}"
	auto_create_subnetworks = "false"
	routing_mode = "REGIONAL"
}

resource "google_compute_subnetwork" "GCP_TEST2_SUBNET" {
	name = "${var.GCP_LOGIN_ID_TAG}-${var.GCP_PROJECT_TAG}-${var.GCP_TEST2_SUBNET_NAME}"
	ip_cidr_range = var.GCP_TEST2_SUBNET_IP_RANGE
...
resource "google_compute_network_peering" "GCP_TEST1_VPC_NETWORK_PEER" {
	name = "${var.GCP_LOGIN_ID_TAG}-${var.GCP_PROJECT_TAG}-${var.GCP_TEST1_VPC_NETWORK_PEER_NAME}"
...
resource "google_compute_network_peering" "GCP_TEST2_VPC_NETWORK_PEER" {
	name = "${var.GCP_LOGIN_ID_TAG}-${var.GCP_PROJECT_TAG}-${var.GCP_TEST2_VPC_NETWORK_PEER_NAME}"
...
resource "google_compute_address" "GCP_VMONE2_IFACE_ETH0_PUBLIC_IP_ADDRESS" {
	name = "${var.GCP_LOGIN_ID_TAG}-${var.GCP_PROJECT_TAG}-${var.GCP_VMONE2_IFACE_ETH0_PUBLIC_IP_ADDRESS_NAME}"
	region = var.GCP_REGION_NAME
	network_tier = "PREMIUM"
	address_type = "EXTERNAL"
}
...
resource "google_compute_instance" "GCP_VMONE2_INSTANCE" {
	name = "${var.GCP_LOGIN_ID_TAG}-${var.GCP_PROJECT_TAG}-${var.GCP_VMONE2_INSTANCE_NAME}"
	zone = var.GCP_ZONE_NAME
	machine_type = "zones/${var.GCP_ZONE_NAME}/machineTypes/${var.GCP_VMONE_MACHINE_TYPE}"
	boot_disk {
		device_name = "persistent-disk-0"
	...
	network_interface {
		network = google_compute_network.GCP_MGMT_VPC_NETWORK.self_link
		network_ip = var.GCP_VMONE2_IFACE_ETH0_PRIVATE_IP_ADDRESS
		subnetwork = google_compute_subnetwork.GCP_MGMT_SUBNET.self_link
		access_config {
	...
	network_interface {
		network = google_compute_network.GCP_TEST2_VPC_NETWORK.self_link
		network_ip = var.GCP_VMONE2_IFACE_ETH1_PRIVATE_IP_ADDRESS
		subnetwork = google_compute_subnetwork.GCP_TEST2_SUBNET.self_link
		access_config {
			network_tier = "PREMIUM"
		}
		alias_ip_range {
			ip_cidr_range = var.GCP_VMONE2_IFACE_ETH1_PRIVATE_IP_ALIASES
		}
	}
	metadata = {
		Owner = var.GCP_OWNER_TAG
